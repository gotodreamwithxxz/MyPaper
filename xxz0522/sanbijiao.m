...
%                            inn=inn+1;
          ...
          for o=rm-1:-1:1
            yy1(list(o))=[];
              ...
              num1=1;
                  for i=1:1:(2*(inn-1))
                      while wq1(i)~=wq2(i) && num1<i/2/Snr/Snr && Snr<28
                           wq1(i)=wq2(i);
                           num1=num1+1;
                      end
                      num22(i)=num1;
              ...
              while w1BER(Snr)>0.0001
                  [latewq1,latewq2]=function2(wq1,wq2); 
                  wBE=0;
...
%grid on
xlim([10,30]);
xlabel('\fontname{宋体}信噪比(dB)','fontsize',10);ylabel('\fontname{宋体}不一致率','fontsize',10)
%title('BER');
legend('本文算法','MAQ算法','CQG算法');
...
xlim([10,30]);
ylim([0,2.6]);
xlabel('\fontname{宋体}信噪比(dB)','fontsize',10);ylabel('\fontname{宋体}密钥生成生成率(bits/probe)','fontsize',10) 
%title('R');
legend('本文算法','CQG算法','MAQ算法');
...
%grid on
xlim([10,30]);
xlabel('\fontname{宋体}信噪比(dB)','fontsize',10);ylabel('\fontname{宋体}总信息率(bits/probe)','fontsize',10) 
%title('LateR');
legend('本文算法','MAQ算法','CQG算法');
...
%grid on
xlim([10,30]);
xlabel('\fontname{宋体}信噪比(dB)','fontsize',10);ylabel('\fontname{宋体}协商阶段信息率','fontsize',10) 
%title('Rate');
legend('本文算法','MAQ算法','CQG算法');
